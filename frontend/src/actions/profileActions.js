    ...
    axios.post(`${serverIp}:${serverPort}/getStudentInfo`, { emailId: eachStudent.studentId })
      .then((response) => {
        ...
        if (response.data === 'Error') {
          ...
          const each = {
            ...response.data, status: eachStudent.status, resumeFileUrl: eachStudent.resumeFileUrl, applyingDate: eachStudent.applyingDate, jobApplicationId: eachStudent._id,
          };
          profiles.push(each);
        }
      }).then(() => dispatch({
