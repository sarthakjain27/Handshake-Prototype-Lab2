...
function handle_request(msg, callback) {
  if (msg.eventName) {
    ...
    company.find({ 'eventPostings.name': { $regex: msg.eventName, $options: 'i' } }, {
      _id: 1, profilePictureUrl: 1, name: 1, eventPostings: 1, emailId: 1,
    }, (err, result) => {
      if (err) {
        console.log(err);
        callback(null, 'Error');
        ...
        result.forEach((eachCompany) => {
          ...
          eachCompany.eventPostings.forEach((eachPosting) => {
