...
function handle_request(msg, callback){
  if(msg.companyName && msg.title){
    ...
    company.findOne({'name':{ $regex: msg.companyName, $options: 'i' }},{_id:1,name:1,jobPostings:1,emailId:1},function(err,result){
      if (err) {
        console.log(err);
        callback(null,'Error');
      } else {
    ...
    company.find({'jobPostings.title':{ $regex: msg.title, $options: 'i' }},{_id:1,name:1,jobPostings:1,emailId:1},function(err,result){
      if (err) {
        console.log(err);
        callback(null,'Error');
      } else {
